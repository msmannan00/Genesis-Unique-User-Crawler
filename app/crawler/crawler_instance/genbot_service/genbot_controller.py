...
class genbot_controller(request_handler):
    ...
    def __init__(self):
        ...
        self.__m_already_crawled = False

    def init(self, p_url):
        __m_host_score = redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_FLOAT, [REDIS_KEYS.RAW_HTML_SCORE + p_url, -1, 60 * 60 * 24 * 10])
        if __m_host_score != -1:
            self.__m_already_crawled = True
    ...
    def validate_duplicate_host_url(self, p_request_url, p_raw_html, p_full_content):
        ...
        if m_hashed_duplication_status is None:
            try:
                g_lock.locked()
                g_lock.acquire()
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_STRING, [m_hash_duplication_key, p_request_url, None])

                files = redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_LIST, [REDIS_KEYS.RAW_HTML_CODE, None, None])
                m_max_similarity = 0
                for html in files:
                ...
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_LIST, [REDIS_KEYS.RAW_HTML_CODE, p_raw_html, None, None])
    ...
    def __trigger_url_request(self, p_request_model: url_model):
        try:
            ...
            if m_response is True:
                m_content = self.__m_html_parser.on_parse_html(m_raw_html, p_request_model)
                return self.validate_duplicate_host_url(p_request_model.m_url, m_raw_html, m_content)
            else:
                return False, False
    ...
    def start_crawler_instance(self, p_request_url):
        ...
        if self.__m_already_crawled is True:
            log.g().w(p_request_url + " : ALREADY CRAWLED")
            return False
